...
def stepV(x):
    return 75*EV*(np.floor(x/BOHR+0.6) - np.floor(x/BOHR+0.4))
...
def solve_tise(V,x_interval,E_interval,mass,verbose=False):
    """ Solve the time-independent Schroedinger equation with hard walls and arbitrary potential.
    
    This function uses the shooting method with a hybridized secant-method solver to solve the
    Schroedinger equation for an arbitrary potential energy function.
    
    Returns tuple of energy and normalized numerical solution (xpoints,ypoints).
    
    V: potential energy function (takes one argument)
    x_interval: (xmin,xmax,steps) -- spatial interval over which solutions should be found
...
#     energy = energy * BOHR**-2 * H_BAR**2 / (2*M_E)
    ...
    energy,numer_soln = solve_tise(squarewellV,(0,BOHR,2000),(100*EV,150*EV),M_E)
    print("\tGround state energy:", energy/EV, "eV")
    pylab.plot(*numer_soln)
    energy,numer_soln = solve_tise(squarewellV,(0,BOHR,2000),(400*EV,600*EV),M_E)
    print("\tFirst excited state energy:", energy/EV,"eV")
    pylab.plot(*numer_soln)
    energy,numer_soln = solve_tise(squarewellV,(0,BOHR,2000),(950*EV,1300*EV),M_E)
    print("\tSecond excited state energy:", energy/EV,"eV")
    pylab.plot(*numer_soln)
    pylab.show()
    
    print("\nInfinite square well with symmetric step:")
    energy,numer_soln = solve_tise(stepV,(0,BOHR,10000),(100*EV,300*EV),M_E)
    print("\tGround state energy:", energy/EV, "eV")
    pylab.plot(*numer_soln)
    energy,numer_soln = solve_tise(stepV,(0,BOHR,10000),(500*EV,800*EV),M_E)
    print("\tFirst excited state energy:", energy/EV, "eV")
    pylab.plot(*numer_soln)
    energy,numer_soln = solve_tise(stepV,(0,BOHR,10000),(1000*EV,1500*EV),M_E)
    print("\tSecond excited state energy:", energy/EV, "eV")
    pylab.plot(*numer_soln)
    pylab.show()
...
#Ex. 8.14 a) Code
    ...
    ho_energy0,ho_soln0 = solve_tise(V,( (-10*a), (10*a), 10000),(100*EV,200*EV),M_E)
    print("\tEnergy:", ho_energy0/EV, "eV")
    
    print("Solving for first excited state:")
    ho_energy1,ho_soln1 = solve_tise(V,( (-10*a), (10*a), 10000),(400*EV,500*EV),M_E)
    print("\tEnergy:", ho_energy1/EV, "eV")
   
    print("Solving for second excited state:")
    ho_energy2,ho_soln2 = solve_tise(V,( (-10*a), (10*a), 10000),(600*EV,700*EV),M_E)
...
#Ex. 8.14 b) Code
    ...
    print("Solving for ground state:")
    anho_energy0,anho_soln0 = solve_tise(V,( (-10*a), (10*a), 10000),(100*EV,300*EV),M_E)
    print("\tEnergy:", anho_energy0/EV, "eV")
    
    print("Solving for first excited state:")
    anho_energy1,anho_soln1 = solve_tise(V,( (-10*a), (10*a), 10000),(400*EV,800*EV),M_E)
    print("\tEnergy:", anho_energy1/EV, "eV")
    
    print("Solving for second excited state:")
    anho_energy2,anho_soln2 = solve_tise(V,( (-10*a), (10*a), 10000),(1000*EV,2000*EV),M_E)
    ...
    print("\nPlotting...")
    anho_energy0,anho_soln0 = solve_tise(V,( (-5*a), (5*a), 10000),(100*EV,300*EV),M_E)
    anho_energy1,anho_soln1 = solve_tise(V,( (-5*a), (5*a), 10000),(400*EV,800*EV),M_E)
    anho_energy2,anho_soln2 = solve_tise(V,( (-5*a), (5*a), 10000),(1000*EV,2000*EV),M_E)

    pylab.plot(*anho_soln0)
    pylab.plot(*anho_soln1)
    pylab.plot(*anho_soln2)
    pylab.show()
